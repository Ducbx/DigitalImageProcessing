...
BEGIN_MESSAGE_MAP(CW03View, CView)
	...
	ON_COMMAND(ID_ZOOMOUT_FORWARDMAPPING, &CW03View::OnZoomoutForwardmapping)
	ON_COMMAND(ID_BACKWARDMAPPING_BMNEARESTNEIGHBOUR, &CW03View::OnBackwardmappingBmnearestneighbour)
	ON_COMMAND(ID_BACKWARDMAPPING_BMBILINEAR, &CW03View::OnBackwardmappingBmbilinear)
END_MESSAGE_MAP()
...
void CW03View::OnZoomoutForwardmapping()
{
	if (mybmp_1.IsEmpty())
	{
		MessageBox("Please read the first picture");
	...
	CTwoRefDlg dlg;
	dlg.DoModal();
	float fXZoomRatio = dlg.ref1;
	float fYZoomRatio = dlg.ref2;

	if (fXZoomRatio <= 0|| fYZoomRatio<=0)
	{
		MessageBox("You have input numbers larger than 0 for zoominng! ");
		return;
	}
	CString cstr;
	cstr.Format("ZoomingRatioX:%f\t%f\n", dlg.ref1,dlg.ref2);
	MessageBox(cstr);
	

	long lNewWidth = (long)(sizeDibDisplay_1.cx*fXZoomRatio);
	long lNewHeight = (long)(sizeDibDisplay_1.cy*fXZoomRatio);
	//flg initial
	bool *flag = new bool[lNewWidth*lNewHeight];
	for (int i = 0; i < lNewWidth*lNewHeight; i++)
	{
		flag[i] = false;
	}
	//mybmp_output.Empty();
	mybmp_output.CreateCDib(CSize(lNewWidth, lNewHeight), 24);
	sizeDibDisplay_output = mybmp_output.GetDimensions();
	...
	for (int y = 0; y < sizeDibDisplay_1.cy; y++)
	{
		...
		for (int x = 0; x < sizeDibDisplay_1.cx; x++)
		{
			...
			//计算前向映射中四个点的位置
			int x0,y0;
			x0 = x*fXZoomRatio+0.5;
			y0 = y*fYZoomRatio+0.5;
			...
			for (int m1 = -1; m1 <= 1; m1++)
				for (int m2 = -1; m2 <= 1; m2++)
				{
					...
					{
						mybmp_output.WritePixel(x0 + m1, y0 + m2, color);
...
void CW03View::OnBackwardmappingBmnearestneighbour()
{
	// TODO: Add your command handler code here
	if (mybmp_1.IsEmpty())
	{
	...
	long lNewWidth = (long)(sizeDibDisplay_1.cx*fXZoomRatio+0.5);
	long lNewHeight = (long)(sizeDibDisplay_1.cy*fXZoomRatio+0.5);
...
void CW03View::OnBackwardmappingBmbilinear()
{
	if (mybmp_1.IsEmpty())
	{
		MessageBox("Please read the first picture");
